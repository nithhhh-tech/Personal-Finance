...
class DashboardController extends Controller
{
    public function summary(Request $request)
    {
        ...
        // Spending by top-level category (for donut chart)
        ...
        // Spending by sub-category (safeguarded if DB migration is pending)
        $spendingBySubcategory = [];
        try {
            if (\Illuminate\Support\Facades\Schema::hasColumn('transactions', 'sub_category_id')) {
                $spendingBySubcategory = $user->transactions()
                    ->selectRaw('sub_category_id, category_id, sum(base_amount) as total')
                    ->where('type', 'expense')
                    ->whereNotNull('sub_category_id')
                    ->whereBetween('transaction_date', [$monthStart, $monthEnd])
                    ->with(['subCategory', 'category'])
                    ->groupBy('sub_category_id', 'category_id')
                    ->orderByDesc('total')
                    ->limit(8)
                    ->get();
            }
        ...
        return [
            ...
            'recent_transactions'      => $user->transactions()->with(['account', 'category', 'subCategory'])->whereDate('transaction_date', $today)->latest()->limit(5)->get(),
            'spending_by_category'     => $spendingByCategory,
            'spending_by_subcategory'  => $spendingBySubcategory,
        ];
    }
}
