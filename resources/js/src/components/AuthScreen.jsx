...
export default function AuthScreen({ onAuthed }) {
    ...
    const authIsOpen = mode === 'login' || mode === 'register';
        ...
        <>
            ...
            {authIsOpen && (
                <AuthDrawer onClose={closeAuth}>
                    <AuthForm error={error} form={form} mode={mode} setForm={setForm} setMode={setMode} submit={submit} submitting={submitting} onClose={closeAuth} />
                </AuthDrawer>
            )}
        </>
    );
...
function LandingPage({ setMode }) {
    ...
    return (
        <main className="h-screen snap-y snap-proximity overflow-y-auto scroll-smooth bg-[#2a1a12] px-4 py-6 text-[#f8efe3] md:snap-mandatory">
            ...
            <section id="home" className="mx-auto grid min-h-[calc(100vh-9.5rem)] scroll-mt-28 snap-start max-w-6xl items-center gap-10 py-10 lg:grid-cols-[1fr_420px]">
                <div>
                    <div className="max-w-3xl">
                        ...
                        <div className="mt-8 flex flex-wrap gap-3">
                            <button type="button" onClick={() => setMode('register')} className="h-12 rounded-md bg-[#d7a86e] px-6 font-bold text-[#2a1a12] hover:bg-[#e8bb82]">Start Tracking</button>
                            <button type="button" onClick={() => scrollToSection('features')} className="h-12 rounded-md border border-[#8f633e] bg-[#3a251a]/70 px-6 font-semibold text-[#fff8ef] hover:bg-[#4a3022]">See Features</button>
                        </div>
                    </div>
                </div>
                <div className="rounded-lg border border-[#8f633e]/60 bg-[#3a251a]/75 p-6 shadow-2xl shadow-black/20">
                    <p className="text-sm font-semibold uppercase tracking-[0.16em] text-[#f2c38b]">Built for daily use</p>
                    <h3 className="mt-3 text-3xl font-bold leading-tight text-[#fff8ef]">Write it down, then know what is left.</h3>
                    <div className="mt-6 space-y-3">
                        <div className="flex items-center justify-between rounded-md border border-[#8f633e]/45 bg-[#2a1a12]/65 px-4 py-3">
            ...
            <section id="features" className="mx-auto grid min-h-[calc(100vh-9.5rem)] scroll-mt-28 snap-start max-w-6xl items-center gap-6 border-t border-[#8f633e]/30 py-8 sm:py-10 lg:grid-cols-[0.85fr_1.6fr]">
                <div>
                    ...
                    <h3 className="mt-3 text-4xl font-bold leading-tight text-[#fff8ef]">Everything for simple daily money tracking.</h3>
            ...
            <section id="flow" className="mx-auto grid min-h-[calc(100vh-9.5rem)] scroll-mt-28 snap-start max-w-6xl items-center gap-6 py-10 lg:grid-cols-[0.85fr_1.6fr]">
                <div>
                    ...
                    <h3 className="mt-3 text-4xl font-bold leading-tight text-[#fff8ef]">A simple flow you can use every day.</h3>
...
function AuthDrawer({ children, onClose }) {
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#120b07]/55 px-4 py-6 backdrop-blur-sm">
...
function AuthForm({ error, form, mode, setForm, setMode, submit, submitting, onClose }) {
    return (
        <form onSubmit={submit} className="w-full rounded-lg border border-[#8f633e]/60 bg-[#3a251a]/95 p-6 text-[#f8efe3] shadow-2xl shadow-black/30 backdrop-blur">
            ...
            <div className="mb-6">
                <p className="text-sm font-semibold uppercase tracking-[0.16em] text-[#f2c38b]">{mode === 'login' ? 'Login' : 'Create account'}</p>
                <h2 className="mt-2 text-3xl font-bold tracking-normal text-[#fff8ef]">{mode === 'login' ? 'Welcome back' : 'Start your money tracker'}</h2>
                <p className="mt-2 text-sm leading-6 text-[#d9c4ad]">{mode === 'login' ? 'Continue tracking your real money records.' : 'Create your private tracker and start with your first wallet.'}</p>
            </div>
            <div className="mb-6 flex rounded-md border border-[#8f633e]/50 bg-[#2a1a12]/60 p-1">
                <button type="button" onClick={() => setMode('login')} className={`flex-1 rounded px-3 py-2 text-sm font-semibold ${mode === 'login' ? 'bg-[#d7a86e] text-[#2a1a12] shadow-sm' : 'text-[#d9c4ad]'}`}>Login</button>
                <button type="button" onClick={() => setMode('register')} className={`flex-1 rounded px-3 py-2 text-sm font-semibold ${mode === 'register' ? 'bg-[#d7a86e] text-[#2a1a12] shadow-sm' : 'text-[#d9c4ad]'}`}>Register</button>
            </div>
            <div className="space-y-4">
                ...
                <Input label="Password" type="password" value={form.password} onChange={(password) => setForm({ ...form, password })} variant="dark" />
            ...
            <button disabled={submitting} className="mt-6 h-12 w-full rounded-md bg-[#d7a86e] px-4 font-bold text-[#2a1a12] shadow-lg shadow-black/20 hover:bg-[#e8bb82] disabled:cursor-not-allowed disabled:opacity-60">{submitting ? 'Please wait...' : mode === 'login' ? 'Login' : 'Create account'}</button>
